...
public class ConfirmedDisposal extends JPanel {
	...
	public ConfirmedDisposal() {
		...
		final JButton cleanDisposeButton = new JButton(
				"Dispose With Confirmation");
		final JButton createWebBrowserButton = new JButton(
		...
		cleanDisposeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (webBrowser.disposeNativePeer(true)) {
					...
					webBrowserPanel.add(new JLabel(
							"The web browser was disposed."),
							BorderLayout.CENTER);
	...
	private void createWebBrowser() {
		...
		webBrowser
				.setHTMLContent("<html>"
						...
						+ LS
						+ "        return 'Here, a normal page would tell you there is some unsaved data.';"
						+ LS
						+ "      };"
						+ LS
						+ "    </script>"
						...
						+ "  <body>"
						+ LS
						+ "    <textarea rows=\"5\" cols=\"25\">A bogus editor...</textarea>"
						+ LS + "  </body>" + LS + "</html>");
	...
	public static void main(String[] args) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				frame.getContentPane().add(new ConfirmedDisposal(),
						BorderLayout.CENTER);
