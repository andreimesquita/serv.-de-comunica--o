 ...
 */
public class XPCOMDownloaderManager extends JPanel {

	public XPCOMDownloaderManager() {
		...
		downloadsPanel
				.setBorder(BorderFactory
						.createTitledBorder("Download manager (on-going downloads are automatically added to this area)"));
		add(downloadsPanel, BorderLayout.SOUTH);
		// We can only access XPCOM when it is properly initialized.
		// This happens when the web browser is created so we run our code in
		// sequence.
		webBrowser.runInSequence(new Runnable() {
			public void run() {
				try {
					nsIComponentRegistrar registrar = MozillaXPCOM.Mozilla
					...
					registrar.registerFactory(NS_DOWNLOAD_CID, "Transfer",
							NS_TRANSFER_CONTRACTID, new nsIFactory() {
					...
					JOptionPane
							.showMessageDialog(
									webBrowser,
									"Failed to register XPCOM download manager.\nPlease check your XULRunner configuration.",
									"XPCOM interface",
	...
	private static nsITransfer createTransfer(final JPanel downloadsPanel) {
		return new nsITransfer() {
			...
			public void init(nsIURI source, nsIURI target, String displayName,
					nsIMIMEInfo MIMEInfo, double startTime,
					nsILocalFile tempFile, final nsICancelable cancelable) {
				downloadComponent = new JPanel(new BorderLayout(5, 5));
				downloadComponent.setBorder(BorderFactory.createEmptyBorder(2,
						5, 2, 5));
			...
			public void onStateChange(nsIWebProgress webProgress,
					nsIRequest request, long stateFlags, long status) {
			...
			public void onProgressChange64(nsIWebProgress webProgress,
					nsIRequest request, long curSelfProgress,
					long maxSelfProgress, long curTotalProgress,
					long maxTotalProgress) {
				long currentKBytes = curTotalProgress / 1024;
				long totalKBytes = maxTotalProgress / 1024;
				downloadStatusLabel.setText(baseText + " (" + currentKBytes
						+ "/" + totalKBytes + ")");
			}

			public void onStatusChange(nsIWebProgress webProgress,
					nsIRequest request, long status, String message) {
			}
			...
			public void onProgressChange(nsIWebProgress webProgress,
					nsIRequest request, int curSelfProgress,
					int maxSelfProgress, int curTotalProgress,
	...
	public static void main(String[] args) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				frame.getContentPane().add(new XPCOMDownloaderManager(),
						BorderLayout.CENTER);
