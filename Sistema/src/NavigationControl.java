	...
	public NavigationControl() {
		...
		webBrowser
				.setHTMLContent("<html>"
						...
						+ "  <body>"
						+ LS
						+ "    <a href=\"http://java.sun.com\">http://java.sun.com</a>: force link to open in a new tab.<br/>"
						+ LS
						+ "    <a href=\"http://www.google.com\">http://www.google.com</a>: force link to open in a new window.<br/>"
						+ LS
						+ "    <a href=\"http://www.eclipse.org\">http://www.eclipse.org</a>: block link. Context menu \"Open in new Window\" creates a new tab.<br/>"
						+ LS
						+ "    <a href=\"http://www.yahoo.com\" target=\"_blank\">http://www.yahoo.com</a>: link normally opens in a new window but creates a new tab.<br/>"
						+ LS
						+ "    <a href=\"http://www.microsoft.com\">http://www.microsoft.com</a>: link and \"Open in new Window\" are blocked.<br/>"
						+ LS + "  </body>" + LS + "</html>");
		webBrowser.addWebBrowserListener(new WebBrowserAdapter() {
			...
			public void locationChanging(WebBrowserNavigationEvent e) {
				...
				} else if (newResourceLocation
						.startsWith("http://java.sun.com/")) {
					e.consume();
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							JWebBrowser webBrowser = new JWebBrowser();
						...
						.startsWith("http://www.eclipse.org/")) {
					e.consume();
				} else if (newResourceLocation
						.startsWith("http://www.microsoft.com/")) {
					e.consume();
				}
			}
				...
				e.getNewWebBrowser().addWebBrowserListener(
						new WebBrowserAdapter() {
							...
							public void locationChanging(
								...
								final JWebBrowser webBrowser = e
										.getWebBrowser();
								webBrowser.removeWebBrowserListener(this);
								String newResourceLocation = e
								...
								if (newResourceLocation
									...
									JWebBrowser.copyAppearance(webBrowser,
											newWebBrowser);
									newWebBrowser.navigate(newResourceLocation);
									tabbedPane.addTab("www.eclipse.org",
											newWebBrowser);
								} else if (newResourceLocation
										.startsWith("http://www.yahoo.com/")) {
									isBlocked = true;
									JWebBrowser newWebBrowser = new JWebBrowser();
									JWebBrowser.copyAppearance(webBrowser,
											newWebBrowser);
									newWebBrowser.navigate(newResourceLocation);
									tabbedPane.addTab("www.yahoo.com",
											newWebBrowser);
								}
								if (isBlocked) {
									e.consume();
									...
									SwingUtilities.invokeLater(new Runnable() {
										public void run() {
											webBrowser.getWebBrowserWindow()
													.dispose();
										}
									});
								}
	...
	public static void main(String[] args) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				frame.getContentPane().add(new NavigationControl(),
						BorderLayout.CENTER);
