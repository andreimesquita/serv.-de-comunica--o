...
public class Cookies extends JPanel {
	...
	public Cookies() {
		...
		final String url = WebServer.getDefaultWebServer()
				.getDynamicContentURL(getClass().getName(), "index.html");
		webBrowser.navigate(url);
		webBrowserPanel.add(webBrowser, BorderLayout.CENTER);
		add(webBrowserPanel, BorderLayout.CENTER);
		JPanel cookiesPanel = new JPanel(new BorderLayout());
		cookiesPanel.setBorder(BorderFactory
				.createTitledBorder("Get/Set Cookies"));
		JPanel getSetNorthPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 4,
				2));
		getSetNorthPanel.add(new JLabel("Cookie Value:"));
		final JTextField setCookieValueField = new JTextField(
				"Some other value", 14);
		getSetNorthPanel.add(setCookieValueField);
		JButton setButton = new JButton("Set");
		setButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				JWebBrowser.setCookie(url, "SomeNSCookie="
						+ setCookieValueField.getText());
			}
		});
		...
		JPanel getSetSouthPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 4,
				2));
		...
		return new WebServerContent() {
			...
			public InputStream getInputStream() {
				String content = "<html>"
						...
						+ LS
						+ "        var value = document.forms['cookieForm'].cookieValue.value;"
						+ LS
						+ "        if (!value) {"
						+ LS
						...
						+ "        }"
						+ LS
						+ "        alert('The value of the cookie is: ' + value);"
						+ LS
						+ "      }"
						+ LS
						...
						+ "  <body>"
						+ LS
						+ "    <form name=\"cookieForm\" action=\"#\"><input name=\"cookieValue\" value=\"Some value\"> <a href=\"javascript:saveCookie('SomeNSCookie')\" class=\"page\">Set cookie</a></form><br>"
						+ LS
						+ "    <a href=\"javascript:readCookie('SomeNSCookie')\" class=\"page\">Read cookie</a>, <a href=\"javascript:deleteCookie('SomeNSCookie')\" class=\"page\">Delete cookie</a>"
						+ LS + "  </body>" + LS + "</html>" + LS;
